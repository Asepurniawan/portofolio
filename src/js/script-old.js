...
const NavigationModule = {
    ...
    openSidebar: function() {
        ...
        // Brief stacked loading overlay when opening the menu on mobile
...
const UtilsModule = {
    showLoadingScreen: function() {
        ...
        const hide = () => {
            ...
            if (firstSection && !firstSection.classList.contains('section-active')) {
                firstSection.classList.add('section-active');
            }
        };

        // Primary hide after short delay for smoothness
        setTimeout(hide, 800);

        // Fallback: ensure hidden on window load
        window.addEventListener('load', hide, { once: true });

        // Hard fallback: remove node entirely if still visible after 5s
        setTimeout(() => {
